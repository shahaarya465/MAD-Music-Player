...
// A simple class to represent a fully loaded song
class Song {
  final String id;
  final String title;
  final String path;
  ...
  // Override equals and hashCode to make Set operations work correctly
  @override
...
class _PlaylistDetailScreenState extends State<PlaylistDetailScreen> {
  ...
  Future<void> _removeSongsFromPlaylist(Set<Song> songsToRemove) async {
    final songIdsToRemove = songsToRemove.map((s) => s.id).toSet();
    widget.playlist.songIDs.removeWhere((id) => songIdsToRemove.contains(id));
    ...
    setState(() {
      _songs.removeWhere((s) => songIdsToRemove.contains(s.id));
      _filteredSongs.removeWhere((s) => songIdsToRemove.contains(s.id));
      _exitSelectionMode();
    });
  }

  Future<void> _deleteSongsGlobally(Set<Song> songsToDelete) async {
    final songIdsToDelete = songsToDelete.map((s) => s.id).toSet();
    ...
    for (var file in playlistFiles) {
      ...
      if (playlist.songIDs.any((id) => songIdsToDelete.contains(id))) {
        playlist.songIDs.removeWhere((id) => songIdsToDelete.contains(id));
        final playlistJson = {
          "name": playlist.name,
  ...
  Future<void> _showMultiDeleteConfirmationDialog() async {
    final count = _selectedSongs.length;
    final actionText = widget.isAllSongsPlaylist
        ? "permanently delete"
        : "remove";
    return showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          title: Text('$actionText $count song(s)?'),
          content: Text(
            'Are you sure you want to $actionText these $count song(s)? This cannot be undone.',
          ),
          actions: [
            TextButton(
            ...
            TextButton(
              child: Text(
                widget.isAllSongsPlaylist ? 'Delete' : 'Remove',
                style: TextStyle(color: Theme.of(context).colorScheme.error),
              ),
              onPressed: () {
                Navigator.of(context).pop();
                if (widget.isAllSongsPlaylist) {
                  _deleteSongsGlobally(_selectedSongs);
                } else {
                  _removeSongsFromPlaylist(_selectedSongs);
                }
              },
            ),
          ],
      ...
      return AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: widget.isAllSongsPlaylist
                ? 'Delete from Library'
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: _buildAppBar(),
      floatingActionButton: _isSelectionMode || widget.isAllSongsPlaylist
          ? null
                              ...
                              : PopupMenuButton<String>(
                                  onSelected: (value) {
                                    if (value == 'rename') {
                                      ...
                                      if (widget.isAllSongsPlaylist) {
                                        _showGlobalDeleteConfirmationDialog(
                                          song,
                                        );
                                      } else {
                                        _removeSongsFromPlaylist({
                                          song,
                                        }); // Pass as a Set
                                      }
                                    }
